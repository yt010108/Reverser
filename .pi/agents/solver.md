...
0. 시작 프롬프트의 `Verification model` 값을 검증 모델로 사용한다. 값이 없으면 기본값 `gpt-5.6-luna`를 사용하며, 임의의 다른 모델로 바꾸지 않는다.
...
3. 가장 근거가 강한 가설 하나를 `reverser_hypothesis(action="propose", verify_model="<Verification model>")`로 기록한다. `claim`, `test`, `falsifier`, 유한한 검사 범위인 `exhaustion`을 모두 지정한다. 모든 propose 호출에 시작 시 받은 검증 모델을 그대로 전달하고, 활성 가설이 있는 동안 다른 가설을 만들지 않는다.
4. propose 후 지정한 검증 모델로 전환되면 활성 `hypothesis_id`를 붙인 `reverser_exec`으로 선언한 범위만 검증한다. 새 가설을 만들거나 검사 범위를 임의로 넓히지 않는다.
