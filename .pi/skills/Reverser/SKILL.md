...
3. 부모 Pi는 반환된 `challenge_id`를 `reverser_solve`에 넘기고 직접 풀이하지 않는다. Solver는 현재 모델과 thinking을 상속한 별도 Pi 프로세스에서 triage부터 Write-up까지 처리한다.
4. `reverser_solve`가 Solver 종료 후 상태를 내부에서 확인한다. `unsolved` 또는 `research_due: true`일 때만 새 컨텍스트의 Reviewer를 자동 실행한다. 부모는 중간 `reverser_status`를 불러오지 않는다.
